...
const SaveConfigBody = z.object({
  ...
  max_valor_empty: z.coerce.number().default(0),
...
configRouter.get("/:instanceId", asyncHandler(async (req, res) => {
  ...
  const cfg = await query<{
    ...
    max_valor: number; max_valor_empty: number; token_strategy: string; token_strategy_n: number;
  ...
  await query(
    `UPDATE instance_configs
     SET category = $2, allowed_categories = $3,
         ...
         max_valor = $11, max_valor_empty = $12, token_strategy = $13, token_strategy_n = $14,
         timing_intra_min_ms = $15, timing_intra_max_ms = $16,
         timing_pause_min_ms = $17, timing_pause_max_ms = $18,
         timing_click_min_ms = $19, timing_click_max_ms = $20,
         clicks_per_org = $21, match_msg_delay_ms = $22,
         match_msg_delay_min_ms = $23, match_msg_delay_max_ms = $24,
         entry_cap_with_players_per_60s = $25,
         entry_cap_empty_per_60s = $26,
         entry_cap_total_per_60s = $27,
         refusal_check_delay_ms = $28,
         hot_org_extra_clicks = $29,
         enable_60rpm_mode = $30,
         active_queue_soft_limit = $31,
         active_queue_hard_limit = $32,
         optimize_for_conversion = $33,
         only_empty_queues = $34,
         updated_at = NOW()
     WHERE instance_id = $1`,
    [id, primaryCategory, allowedCategoriesStr,
     ...
     blocked_names ?? "",
     Number(max_valor ?? 0), Number(max_valor_empty ?? 0),
     safeStrategy, Math.max(1, Number(token_strategy_n ?? 5)),
